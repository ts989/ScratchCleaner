...
if [[ -s /scratch/for_del ]] ; then 
        cat /scratch/for_del | mail -s "$tag Files for deletion on $(hostname)" "Tomislav.Stilinovic@srce.hr"
	if [[ -s /scratch/rescue_me ]] ; then
		cat /scratch/rescue_me | mail -s "$tag Files for rescuing on $(hostname)" "Tomislav.Stilinovic@srce.hr"  
	fi
